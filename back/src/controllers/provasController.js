...
const ProvasController = {
    ...
    async listarPorVestibular(req, res) {
        try {
            const { id_vestibular } = req.params
            const dados = await ProvasModel.listarPorVestibular(id_vestibular)
            res.status(200).json(dados)
    ...
    async deletar(req, res) {
        try {
            ...
            res.status(200).json({
                resposta: "Prova deletada com sucesso"
            })
